...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	// 将perm去掉 PTE_COW，加上 PTE_D
	perm = (perm & ~PTE_COW) | PTE_D;

	/* Step 3: Allocate a new page at 'UCOW'. */
	/* Exercise 4.13: Your code here. (3/6) */
	// 申请一页新的物理页，该物理页对应的虚拟地址为 UCOW,改地址之上的PAGE_SIZE
	// 大小空间专门用于写时复制时申请新的物理页。
	syscall_mem_alloc(0, (void *)UCOW, perm);

	/* Step 4: Copy the content of the faulting page at 'va' to 'UCOW'. */
	/* Hint: 'va' may not be aligned to a page! */
	/* Exercise 4.13: Your code here. (4/6) */
	// 将 va 所在物理页的内容复制到新申请的页面中
	memcpy((void *)UCOW, (void *)ROUNDDOWN(va, PAGE_SIZE), PAGE_SIZE);

	// Step 5: Map the page at 'UCOW' to 'va' with the new 'perm'.
	/* Exercise 4.13: Your code here. (5/6) */
	// 将当前进程的 UCOW 所在的物理页，作为当前进程的 va 地址所映射的物理页
	// 在这个过程中就自动取消了从va到原物理页的映射
	...
	// 使用系统调用 syscall_set_trapframe，将Trapframe设置成tlb_mod开始时的
	// trapfram，这样当系统调用结束时就会返回到原异常代码处了。
...
int fork(void) {
	...
	// correct value.
	child = syscall_exofork(); // 一次调用，两次返回
	if (child == 0) {
		env = envs + ENVX(syscall_getenvid()); // env和envs是位于用户空间的
		return 0;
	}
