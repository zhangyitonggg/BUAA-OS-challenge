...
int syscall_get_env_parent_id(u_int envid) {
	return msyscall(SYS_get_env_parent_id, envid);
}

void syscall_add_job(u_int envid, char *cmd) {
    msyscall(SYS_add_job, envid, cmd);
}
...
void syscall_try_done_job_status(u_int envid) {
	msyscall(SYS_try_done_job_status, envid);
}

void syscall_kill_job(u_int job_id) {
	msyscall(SYS_kill_job, job_id);
}

u_int syscall_get_envid_by_job(u_int job_id) {
	return msyscall(SYS_get_envid_by_job, job_id);
}