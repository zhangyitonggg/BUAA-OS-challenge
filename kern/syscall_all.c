...
int sys_exofork(void) {
	...
	// curenv->env_tf 存储的是进程调度，切换为其他进程之前的 trap frame。
	// 而父进程调用 syscall_exofork 时保存的现场，并不一定等同于curenv->tf 中的信息。
 ...
 *	* -----------+------------+--------*
 *	|  console   | 0x180003f8 | 0x20   | 终端
 *	|  IDE disk  | 0x180001f0 | 0x8    | 磁盘
 *	* ---------------------------------*
 */
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_try_done_job_status] = sys_try_done_job_status,
	[SYS_kill_job] = sys_kill_job,
	[SYS_get_envid_by_job] = sys_get_envid_by_job, 
};
