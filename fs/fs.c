...
// 判断编号blockno对应的磁盘块是否已经被映射到一个真实的物理地址，返回其虚拟地址，没有则返回null
...
int read_block(u_int blockno, void **blk, u_int *isnew) {
	...
	// 通过block_is_mapped判断编号对应的磁盘块是否已经被映射
...
// 解除磁盘块和物理内存之间的映射关系，回收内存,并且会将内存中对磁盘块的修改写回磁盘
...
// 在磁盘块管理位图上找到空闲的磁盘块，更新位图并将位图写入内存,返回磁盘块编号
int alloc_block_num(void) {
	...
	for (blockno = 3; blockno < super->s_nblocks; blockno++) {
		if (bitmap[blockno / 32] & (1 << (blockno % 32))) { // the block is free
			bitmap[blockno / 32] &= ~(1 << (blockno % 32));
			// 申请第blockno个磁盘块，那么也就是位图中第blockno / BLOCK_SIZE_BIT个磁盘块
			// 改变，也就是disk中第blockno / BLOCK_SIZE_BIT + 2个磁盘块
...
int alloc_block(void) {
	...
	// 调用 alloc_block_num 在磁盘块管理位图上找到空闲的磁盘块
	...
	if ((r = map_block(bno)) < 0) {
		free_block(bno); // 出现错误，将申请的block释放
		return r;
	}
...
// 获取文件f中使用的第filebno个磁盘块对应的磁盘块的指针,如果block未申请但是alloc非0的话则需要申请
int file_block_walk(struct File *f, u_int filebno, uint32_t **ppdiskbno, u_int alloc) {
	...
	if (filebno < NDIRECT) { 
		...
		if (f->f_indirect == 0) { // f_indirect未申请
			if (alloc == 0) { // 返回错误信息
				return -E_NOT_FOUND;
			}
...
// 将某个指定的文件指向的磁盘块读入内存,获取文件中第filebno个磁盘块，写入*blk
int file_get_block(struct File *f, u_int filebno, void **blk) {
	...
	// 获取了文件中使用的第filebno个磁盘块对应的磁盘块编号diskbno
...
// 找到指定目录dir下的指定名字name的文件,写入*file
...
int walk_path(char *path, struct File **pdir, struct File **pfile, char *lastelem) {
	...
	*pfile = 0; // 初始化
	...
	while (*path != '\0') {
		...
		name[path - p] = '\0';
		path = skip_slash(path); // 跳过下划线
		if (dir->f_type != FTYPE_DIR) { // 如果当前扫描到的文件不是目录
			return -E_NOT_FOUND;
		}

		// 在当前目录下寻找扫描到的文件名所对应的文件
		if ((r = dir_lookup(dir, name, &file)) < 0) {
			// 如果目标文件所在的目录是存在的，但是目标文件不在
			if (r == -E_NOT_FOUND && *path == '\0') { 
				if (pdir) { // 这时候*pdir还应该如常设置
					*pdir = dir;
				}

				if (lastelem) { // 将目标文件名name拷贝到lastelem中
...
// 将指定文件的大小截断为指定长度;变⼤时候不申请新的block，只是更改f_size，变⼩时会clear多余的block
...
int file_set_size(struct File *f, u_int newsize) {
	if (f->f_size > newsize) {
		file_truncate(f, newsize); // 将f的大小截断为newsize
	}
	...
	if (f->f_dir) {
		file_flush(f->f_dir);  // 将dirty的block写回
	}
...
void file_flush(struct File *f) {
	...
	for (bno = 0; bno < nblocks; bno++) {
		// 获取bno所对应的磁盘块在disk中的编号diskno
		if ((r = file_map_block(f, bno, &diskno, 0)) < 0) {
			continue;
		}
		// 判断disk中第diskno个block是否被修改
